...
class FlightstatusSpider(scrapy.Spider):
    name = 'flightStatus'
    ...
    for kode in kodeBandara:
        for j in jam:
            ...
            payload = ''
            headers = {}
            api = '/v2/api-next/flight-tracker/arr/'+kode+'/'+tanggal_yang_diambil+'/'+j+'?carrierCode=&numHours=6'
            ...
            def parse(self, response):
                ...
                #Masukin ke DB
                sql = "INSERT arrival VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE status=%s, status_add=%s, keberangkatan_tanggal=%s, keberangkatan_jam=%s, kedatangan_tanggal=%s, kedatangan_jam=%s, waktu_extract=%s"
                baris = (
                    id_penerbangan,
                    id_pesawat, 
                    nama_pesawat, 
